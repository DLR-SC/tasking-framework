...
TEST_F(TestFifoReader, OneReaderSevenMessages)
{
    ...
    ASSERT_TRUE(returnvalue); // check for nullptr pointer
...
TEST_F(TestFifoReader, OneReaderFullCycle)
{
    ...
    int8_t value = 41;
    fifo.push(value);
    EXPECT_TRUE(nullptr == reader1.pop());
    fifo.synchronizeStart(&task1, 1);
    int8_t* element = reader1.pop();
    ASSERT_FALSE(nullptr == element);
...
TEST_F(TestFifoReader, NotificationDuringRead)
{
    fifo.associateReader(reader1);
    fifo.push(1);
    fifo.push(2);
