...
#include <stddef.h> // Fix unresolved NULL
#include <assert.h>
...
FifoGenericReader::FifoGenericReader(Tasking::Task* p_task) :
    nextReader(NULL), readerId(0), readerTask(p_task), fifo(NULL), fifo_first(NULL), fifo_last(
    NULL), allocated_elements(NULL), unusedLinks(NULL) {
  ...
  if (unusedLinks == NULL) {
    ...
    links[maximumFifoReaderLinks - 1].next = NULL;
    unusedLinks = links;
  }
  linkMutex.leave();
}
...
bool FifoGenericReader::isEmpty(void) const {
  return (fifo_first == NULL);
}

// -------------------

void FifoGenericReader::release(void* data) {
  // If not associated to a FIFO or no elements are popped, than do nothing.
  if ((fifo == NULL) || (allocated_elements == NULL)) {
    return;
  }
  if (allocated_elements->fifoElement->data == data) {
    ...
    linkMutex.leave();
  } else { // Not head of allocated elements
	   // Find the data in the list of allocated elements
    Chain* previous = allocated_elements;
    while ((previous->next != NULL) && (previous->next->fifoElement->data != data)) {
...
void* FifoGenericReader::pop(void) {
  if ((fifo == NULL) || (fifo_first == NULL)) {
    return NULL;
  }
  ...
  if (link->fifoElement == fifo_last) {
    ...
    assert(fifo_first != NULL);
  }
  return link->fifoElement->data;
}
// -------------------

void FifoGenericReader::releaseAll(void) {
  // Loop over allocated elements and release them in FIFO
  linkMutex.enter();
  while (allocated_elements != NULL) {
    fifo->releaseElement(allocated_elements->fifoElement, readerId);
    Chain* next = allocated_elements->next;
    allocated_elements->next = unusedLinks;
    unusedLinks = allocated_elements;
...
FifoGeneric::FifoGeneric(FifoGeneric::Chain* chain, void* dataBuffer, const size_t size, const unsigned int items) :
    m_unused(NULL), m_allocated(NULL), m_fifo_first(NULL), m_fifo_last(
    NULL), m_lastPushed(NULL), m_readers(
    NULL), m_readerBitMask(1 << 0) {
...
void* FifoGeneric::allocate(void) {
  void* result = NULL;
  m_mutex.enter();
...
void FifoGeneric::release(const void* data, unsigned int readerId) {
...
void FifoGeneric::releaseElement(FifoGeneric::Chain* link, unsigned int readerId) {
  if (link != NULL) {
    assert(m_fifo_first != NULL);
	...
	while ((previous != NULL) && (previous->next != link)) {
	  previous = previous->next;
	}
...
bool FifoGeneric::push(const void* data) {
  bool result = false;
  Chain* element = NULL;
...
const void* FifoGeneric::getlastPushed(void) const {
  return m_lastPushed;
}

// -------------------

bool FifoGeneric::isEmpty(void) const {
  return (m_fifo_first == NULL);
}

// -------------------

FifoGeneric::Chain* FifoGeneric::findPrevious(const void* data) const {
  Chain* result = NULL;
  ...
  if (m_allocated->data != data) {
    ...
    for (Chain* previous = m_allocated; (previous->next != NULL) && (result == NULL); previous = previous->next) {
...
void FifoGeneric::associateReader(FifoGenericReader& reader) {
  m_readerMutex.enter();
  // Check if reader is in the list of the readers. In this case do nothing because it shall unique.
  for (FifoGenericReader* check = m_readers; check != NULL; check = check->nextReader) {
    if (check == &reader) {
      m_readerMutex.leave();
      return;
    }
  ...
  m_mutex.enter();
  for (Chain* element = m_fifo_first; element != NULL; element = element->next) {
...
void FifoGeneric::releaseReader(FifoGenericReader& reader) {
  ...
  if (&reader == m_readers) {
    ...
    for (FifoGenericReader* currentReader = m_readers; currentReader != NULL; currentReader =
...
void FifoGeneric::synchronize(const Tasking::Task* task, const unsigned int volume) {
  ...
  if (volume > 0) {
	...
	if (reader->fifo_first == NULL) {
	  ...
	  // Check if the bit in the reader bit mask was already cleared -> element was already consumed by the reader
	  // follow the chain to the first not set bit
	  while ( /*(reader->fifo_first != NULL) && */((reader->fifo_first->expectedReadsBitMask & reader->readerId)
	      == 0)) {
	    reader->fifo_first = reader->fifo_first->next;
	    assert(reader->fifo_first != NULL);
	  }
