...
public:
    ...
    struct Chain
    {
        ...
        /// One data item
        void* data;
        /// Number of remaining read calls from FIFO reader. Release in a reader is only possible when the number is
        /// zero.
...
private:
    ...
    /// Bitmask of readers associated with this FIFO. To simplify logic the first bit is always set to 1 without
    /// associated readers.
    unsigned char m_readerBitMask;
    /// Mutex to synchronize access to reader list. The mutex is static because access to the list of readers are
    /// seldom.
...
protected:
    ...
    static const int maximumFifoReaderLinks =
            2000; // TODO this shouldn't be a constant and needs to be the same as fifo
